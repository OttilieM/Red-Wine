...
bar <- results %>%
  ...
  theme_bw()
bar
fig.cap <- "Fig 1. Model Performance"
```
...
An Importance plot was used  to show which variables are the most important for determining wine quality.

```{r, echo=F, include=TRUE}
varImpPlot(rf_wine, main = "Random Forest Variable Importance")
fig.cap <- "Fig 2. Variable Importance for Quality"
```
...
Histograms initially showed that the distribution of wine quality is similar in red and white wines. White wine quality has a gaussian skew, whereas red wine quality showed a very slight positive skew. No wines scored above 9 and lower than 3.
...
fig.cap <- "Fig 3. White Wine Quality Distribution"
...
fig.cap <- "Fig 4. Red Wine Quality Distribution"

```

To investigate this further a corrograph was plotted for white and red wine to see if there were any obvious differences in correlation.

```{r, echo=FALSE, include=TRUE}
corplotwhite <- cor(white)
corrplot(corplotwhite, method = "color", type = "upper", main = "White wine")
fig.cap <- "Fig 4. Correlations in White Wine"
```

```{r, echo=FALSE, include=TRUE}
corplotred <- cor(red)
corrplot(corplotred, method = "color", type = "upper", main = "Red wine")
fig.cap <- "Fig 5. Correlations in Red Wine"
```
...
```{r echo=F, warning=FALSE, include=TRUE}
par(mfrow= c(1,2))
plot(prune.white, main = "White wine") #pruned tree better accuracy
text(prune.white, pretty = 0)
fig.cap <- "Fig 6. White Wine Tree"

plot(tree.red, main = "Red wine") #unpruned tree better accuracy
text(tree.red, pretty = 0)
fig.cap <- "Fig 7. Red Wine Tree"
```

The decision trees are clearly quite different between red and white wines, although are both still showing that alcohol is the most important variable for predicting quality.
...
fig.cap <- "Fig 8. Variable Importance for White Wine"
...
fig.cap <- "Fig 9. Variable Importance for Red Wine"
```

Both of the importance plots also highlight how important the alcohol variable is in predicting wine quality, however, there are differences.
